...
const {fetchTags, storeTag, updateTag, deleteTag} = handleTag();
const lists = ref([])
...
const saveUpdateTag = () => {
  const res = isEdit.value ? updateTag(formData.value._id, formData.value) : storeTag(formData.value)
  res.then(({data}) => {
    getList()
    formReset()
    isEdit.value = false;
